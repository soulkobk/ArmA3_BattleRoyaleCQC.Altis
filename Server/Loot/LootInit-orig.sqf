...
if (isServer) then
{
	...
	diag_log format ["<BRMINI>: %1 SPAWNING LOOT IN %2 BUILDINGS...",BRMini_CurrZoneName,(count _houseList)];
				...
				if (_houseNumPos >= 1) then
				{
					for "_n" from 0 to (_houseNumPos - 1) do
					{
						...
						if !(_buildingPos isEqualTo [0,0,0]) then
						{
							...
							{
								[_buildingPos,_showLoot] execVM "Server\Loot\spawnLootInside.sqf";
							};	
						};
					};
				};
				...
				if (_houseNumPos == 0) then 
				{
					for [{_n = 0},{_n < 3},{_n = _n + 1}] do
					{
						...
						while {!_buildingPosOK} do
						{
							...
							{
								_buildingPosOK = true;
								_buildingPlacedObject = createVehicle ["Land_WoodenLog_F",[(_buildingPos select 0),(_buildingPos select 1),((_buildingPos select 2) + 0.5)], [], 0, "CAN_COLLIDE"];
						...
						if !(_buildingPos isEqualTo [0,0,0]) then
						{
							...
							{
								[_buildingPos,_showLoot] execVM "Server\Loot\spawnLootOutside.sqf";
							};	
						};
					};
					{ deleteVehicle _x; } forEach nearestObjects [_house,["Land_WoodenLog_F"],100];
				};
				//
		};
		uiSleep 0.1; // need a sleep so no red chain appears?
	} forEach _houseList;
	
	_waitTime = time + 5;
	waitUntil {time >= _waitTime}; // wait 5 seconds

	diag_log format ["<BRMINI>: %1 FINISHED SPAWNING LOOT",BRMini_CurrZoneName];

/*
	File: start_von.sqf
	Description: Realistic Weather Simulation for BRGH <- NICE COPY/PASTE THERE!
...
["VON_ESP", "onEachFrame", {
	{
		if(_x getVariable ["Speaking",false]) then {
			...
			_eyePos = [((ASLToAGL (eyePos _x)) select 0), ((ASLToAGL (eyePos _x)) select 1), ((ASLToAGL (eyePos _x) select 2)) + 0.4];
			
			// drawIcon3D["",[1,1,1,1],_eyepos,0.1,0.1,45,format["%1",name _x],1,_fontsize,'EtelkaNarrowMediumPro'];
			// drawIcon3D["a3\ui_f\data\gui\Rsc\RscDisplayArsenal\voice_ca.paa",[1,1,1,1],_eyepos,1,1,0,"",1,_fontsize,'EtelkaNarrowMediumPro'];
			drawIcon3D["a3\ui_f\data\gui\Rsc\RscDisplayArsenal\voice_ca.paa",(getArray(configFile >> "CfgInGameUI" >> "SideColors" >> "colorFriendly")),_eyePos,1,1,0,"",1,_fontSize,'EtelkaNarrowMediumPro'];
		};

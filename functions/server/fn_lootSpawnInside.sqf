...
if (isServer) then
{
	...
	// _barrel = createVehicle ["Land_WoodenLog_F",[_pos0,_pos1,_pos2 + 0.5], [], 0, "CAN_COLLIDE"];
	// _barrel setVelocity [0,0,0.5];
	// uiSleep 0.5;
	// _pos2 = (((getPosATL _barrel) select 2) + 0.02); // adjust it up slightly
	// deleteVehicle _barrel;
	// _holder = createVehicle ["groundWeaponHolder",[_pos0,_pos1,_pos2], [], 0, "CAN_COLLIDE"];

	// OWP Edit - fix floating loot
	_intersectDetails = lineIntersectsSurfaces [ATLToASL _pos, [(_pos select 0),(_pos select 1),-1000]];
	_pos = ASLToATL ((_intersectDetails select 0) select 0);
	_pos set [2,(_pos select 2) + 0.02];
